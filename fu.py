...
class HuYouJia_GameSprites(pygame.sprite.Sprite):
...
#背景类
class HuYouJia_Background(HuYouJia_GameSprites):
...
#飞机类
class HuYouJia_Hero(HuYouJia_GameSprites):
	def __init__(self):
		image_name = "./images/32ce7ee507daf21ea59f61dd10a430fe_meitu_1.png" 
		super().__init__(image_name)
		self.rect.centerx = SCREEN_RECT.centerx -130
	...
	def fire(self):
		for i in range(1,4):
			self.bullet = HuYouJia_Bullet()
			#设置精灵位置
			self.bullet.rect.left = self.rect.right+ 15*i
			self.bullet.rect.centery= self.rect.centery
			
			#将精灵添加到精灵组
			self.bullet_group.add(self.bullet)

class Hero2(HuYouJia_GameSprites):
	def __init__(self):
		image_name = "./images/32ce7ee507daf21ea59f61dd10a430fe_meitu_1.png" 
		super().__init__(image_name)
	...
	def fire(self):
		for i in range(1,4):
			self.bullet2 = HuYouJia_Bullet()
			#设置精灵位置
			self.bullet2.rect.left = self.rect.right+ 15*i
...
#敌机类
class HuYouJia_Enemy(HuYouJia_GameSprites):
	def __init__(self):
		image_name = "./images/_meitu_8.png"
		super().__init__(image_name)
		self.speed = random.randint(2,5)
		#设置敌机随机初始位置
		self.rect.x = SCREEN_RECT.width + self.rect.left
...
class HuYouJia_Bullet(HuYouJia_GameSprites):
	def __init__(self):
